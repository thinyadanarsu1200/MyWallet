    ...
    <thead class="bg-gray-100">
      <tr>
        <th scope="col" data-field="name" class="cursor-pointer px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
          <div class="flex">
            @php
              $remove_icon = array_intersect($users->pluck('id')->toArray(), $users_id) && array_diff($users->pluck('id')->toArray(), $users_id) ? 'remove-check' : '';
              $check_icon = array_intersect($users->pluck('id')->toArray(), $users_id) && !count(array_diff($users->pluck('id')->toArray(), $users_id)) ? 'checked' : '';
            @endphp
        ...
        <th scope="col" data-field="ip" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
          IP
        </th>
        <th scope="col" data-field="user_agent" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
          User Agent
        </th>
        <th scope="col" data-field="login_at" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
          Login At
        </th>
        ...
        <tr class="bg-white even:bg-gray-100">
          <td class="px-6 py-4 whitespace-nowrap">
            <div class="flex items-center">
              <div class="flex items-center">
                <input data-id="{{ $user->id }}" {{ in_array($user->id, $users_id) ? 'checked' : '' }} type="checkbox" class="local-check rounded-sm bg-gray-100 mr-4">
              </div>
              @php
              ...
              <x-dropdown2 class="ml-0 lg:hidden" direction="left" position="{{ $position ?? '' }}">
                ...
                <x-dropdown2-link href="#" class="delete-one-dropdown" data-id="{{ $user->id }}" data-name="{{ $user->name }}">Delete</x-dropdown2-link>
              </x-dropdown2>
          ...
          @if ($user->ip)
            <td class="px-6 py-4 whitespace-nowrap">
              <div class="text-sm text-gray-900">{{ $user->ip }}</div>
            </td>
          @else
            <td class="px-6 py-4 whitespace-nowrap">
          ...
          <td class="px-6 py-4 whitespace-nowrap">
            @if ($user->login_at)
              <div class="text-sm text-gray-900">
                {{ date('Y-m-d g:i:a', strtotime($user->login_at)) }}
          ...
          <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
            <div class="flex items-center justify-end flex-nowrap gap-x-1">
              ...
              <button class="text-red-600 hover:text-red-900 flex items-center mr-1">
                <svg xmlns="http://www.w3.org/2000/svg" class="delete-one h-4 w-4" data-id="{{ $user->id }}" data-name="{{ $user->name }}" fill="none" viewBox="0 0 24 24"
                  stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
