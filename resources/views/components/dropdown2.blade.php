...
<div x-data="{ open: false }" @click.outside="open = false" {{ $attributes->merge(['class' => 'relative border-none ring-white']) }}>
  ...
  <div x-show="open"
    ...
    class="origin-top-right absolute z-50 {{ $direction }}-0 {{ $position }} mt-2 w-48 rounded-lg shadow-lg py-1 bg-white ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu"
